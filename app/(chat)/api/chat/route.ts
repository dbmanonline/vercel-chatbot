...
import { verifyGrounding, formatGroundingMessage } from "@/lib/mcp/grounding";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (MCP_ENABLED && MCP_SERVER_URL && MCP_AUTH_TOKEN.length >= 32) {
      try {
        ...
        // Continue without MCP - tools will fall back to defaults
      }
    }

    const mcpTools = mcpClient && mcpTransport
      ? buildMcpTools({
          client: mcpClient,
    ...
    const stream = createUIMessageStream({
      execute: async ({ writer: dataStream }) => {
        ...
        // MCP cleanup - close request-scoped client/transport after stream setup
        // (the actual close happens when result finishes; this is the safety net)
      },
    ...
    // MCP cleanup - close request-scoped client/transport after stream response
    if (mcpClient && mcpTransport) {
      closeMcp(mcpClient, mcpTransport).catch((err) => {
